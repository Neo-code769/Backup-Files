...
$SourcePaths = $SourcePaths ?? @("C:\Windows\Temp")
$BackupDestination = $BackupDestination ?? "C:\Backup"
$LogFile = $LogFile ?? "$PSScriptRoot\backup.log"
$MinFreeGB = $MinFreeGB ?? 5
$RetentionDays = $RetentionDays ?? 15
$EnableCompression = $EnableCompression ?? $true
$EnableDryRun = $EnableDryRun ?? $false
$EnableMail = $EnableMail ?? $false
...
# Nouveaux paramètres
$BackupMode = $BackupMode ?? "Complete" # Options: "Complete", "Differential", "Incremental"
$EnableIntegrityCheck = $EnableIntegrityCheck ?? $true
$ParallelProcessing = $ParallelProcessing ?? $false
$MaxParallelJobs = $MaxParallelJobs ?? 4
$RetryCount = $RetryCount ?? 3
$RetryDelay = $RetryDelay ?? 30  # secondes
$CollectMetrics = $CollectMetrics ?? $true
$MetricsOutputPath = $MetricsOutputPath ?? "$PSScriptRoot\metrics.csv"
$UseSecureCredentials = $UseSecureCredentials ?? $false
$CredentialName = $CredentialName ?? "BackupSMTP"
...
# =============================
Function Create-IntegrityManifest {
    param(
        [Parameter(Mandatory=$true)]
        [string]$BackupPath
    )
...
# =============================
Function Prepare-ReferenceData {
    param(
        [Parameter(Mandatory=$true)]
        [ValidateSet("Complete", "Differential", "Incremental")]
        [string]$BackupMode
    ...
    do {
        try {
            ...
            foreach ($file in $sourceFiles) {
                ...
                $shouldBackup = Should-BackupFile -File $file -Mode $BackupMode -ReferenceData $ReferenceData
        ...
        if ($result.Success -and $EnableIntegrityCheck) {
            ...
            if (Test-Path $targetPath) {
                if (-not $EnableCompression) {
                    Create-IntegrityManifest -BackupPath $targetPath
...
Function Start-Backup {
    ...
    try {
        ...
        $diskSpaceOk = Check-FreeSpace -DriveLetter $driveLetter -MinFreeGB $MinFreeGB
        ...
        $referenceData = Prepare-ReferenceData -BackupMode $BackupMode
